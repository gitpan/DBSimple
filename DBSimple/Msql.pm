...
DBSimple::Msql - C<DBSimple> implementation class for Msql v1 databases
...
This class attempts to provide permissible equivalents for statements
that aren't directly supported by Msql, but will die if you attempt to
exceed certain limits : for example, if you do
	...
	my %types = (
		'char' => {
			...
			'write' => sub {
				...
				warn "String ('$_[1]') longer than field ($_[0]->{size})"
					if length $_[1] > $_[0]->{size};
				Msql->quote($_[1]);
			},
		},
		'int' => {
...
sub clike {
	...
	my $field = shift->fieldname;
	my $match = shift;
	$match =~ s/([a-zA-Z])/[\u$1\l$1]/g;
	$match = Msql->quote($match);
	$self->sqlmap($fields, <<SQL);
select @{[ join ", ", map $_->fieldname, @$fields ]}
	from $table @{[ $self->somewhere(@_) ]} $field like $match
SQL
